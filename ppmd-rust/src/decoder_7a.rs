...
use ppmd_sys::{
    ...
    PPMD7_SYM_END, Ppmd7_Alloc, Ppmd7_Construct, Ppmd7_Free, Ppmd7_Init, Ppmd7a_DecodeSymbol,
...
impl<R: Read> Ppmd7aDecoder<R> {
    ...
    pub fn new(reader: R, order: u32, mem_size: u32) -> crate::Result<Self> {
        ...
        let success = unsafe { Ppmd7a_RangeDec_Init(&mut ppmd.rc.dec) };
...
impl<R: Read> Drop for Ppmd7aDecoder<R> {
    fn drop(&mut self) {
    ...
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        unsafe {
            for byte in buf.iter_mut() {
                sym = Ppmd7a_DecodeSymbol(&mut self.ppmd);
...
#[cfg(test)]
mod test {
    use super::Ppmd7aDecoder;

    const ORDER: u32 = 8;
    const MEM_SIZE: u32 = 262144;

    #[test]
    fn ppmd7adecoder_init_drop() {
        let reader: &[u8] = &[];
        let decoder = Ppmd7aDecoder::new(reader, ORDER, MEM_SIZE).unwrap();
